...
하이클래스 등 학교 알림장 글을 달력 화면에 붙여넣으면, 본인 Anthropic 키로 **날짜 있는 일정**과
**준비물·제출 같은 할 일**을 뽑아 **확인 후 선택 등록**한다(자동으로 무단 추가하지 않음).

- `lib/notice-ai.ts` — 구조화 출력(`output_config.format`)으로 `{events, todos}` 추출. 상대 날짜(내일/이번 주 금요일)는 오늘 기준으로 환산
- `components/NoticeImport.tsx` — 붙여넣기 → 뽑기 → 체크박스 선택 → `addEvent`/`addTodo` 등록
- 하이클래스 자동 연동은 공개 API 부재 + 정적 PWA 제약으로 불가 → 붙여넣기 방식으로 대체
