...
하단 탭 6개로 구성된다. **첫 화면(루트 `/`)은 달력** — 앱을 열면 일정부터 보인다.

- **달력** (`/`) — 월 뷰 + 일자별 마커 + 일정 추가/완료. 첫 화면.
- **오늘** (`/today`) — 에디 시계, 출발 카운트다운, 오늘 약, 오늘 일정, 아침 루틴
...
`/calendar`는 예전 "친구에게 일정 공유" 링크(`…/calendar/#sched=…`) 호환용 리다이렉트만 한다 — 해시를 그대로 들고 `/`로 넘어간다. 새로 만드는 공유 링크는 `/#sched=…`.
...
- **큰 일 쪼개기(`BreakdownCard`, BYOK)** — 막막한 할 일을 AI가 5분 단위 첫 단계로 분해 → 선택 등록(`lib/breakdown-ai.ts`).
- **기분 1탭 기록** — 저녁 회고에 😀🙂😔, `moods`에 저장.
- **백업/복원** — 데이터를 JSON으로 내보내고 파일로 복원(`importState`, 기기 옮길 때). API 키는 보안상 제외되고 기기에 그대로 유지.

## 반복 알림 · 우선순위

- **반복 알림(`/more/recurring`, `Recurring`)** — 매일/매주 정해진 시각, 또는 **N분 간격(물·움직임 넛지)**. 스케줄러가 시각 일치/간격 슬롯에 발송(총량 상한 적용). 프리셋(물 2시간·스트레칭 1시간·비타민) 원탭.
- **우선순위(★)** — 할 일에 '오늘 꼭' 표시, 목록 상단 정렬. `NextOne`이 우선 할 일을 먼저 추천.

## 지금 뭐 하지? · 저녁 회고 · 에디 성장

- **지금 뭐 하지?(`NextOne`)** — 결정 마비를 줄이려 오늘 마감→시각 일정→우선(★) 할 일→알림 할 일→그 외 할 일 순으로 **딱 하나**를 추천하고 바로 🎯집중/끝냈어.
- **저녁 1탭 회고(`EveningReview`)** — "오늘 한 것 하나 + 내일의 한 가지"를 `reviews`에 저장(자책 없는 마무리).
- **에디 성장(`eddieStage`)** — 누적 '잘 보낸 하루'(streak.total)에 따라 🥚→🐣→🐥→🐔 단계, 에디 화면에 단계·남은 일수 표시(높은 단계엔 볏).
...
'할 일 빨리 담기'에 **'내일', '다음주 화요일', '6/27 3시', '한달 뒤'** 같은 표현이 있으면 캘린더 일정으로 넣는다.

- `lib/parse-when.ts` — 규칙 기반 한국어 날짜/시각 파서(오늘/내일/모레, 이번주·다음주·다다음주 요일, M월D일, 다음달 D일, M/D, HH:mm, 오전·오후 N시반). 표현은 제목에서 제거.
- **상대 기간 표현**: 'N일/N주/N개월(N달)/N년 뒤·후'(숫자·한글 수사 '한 달' 등 모두 인식), '일주일 뒤'(=7일), '보름 뒤'(=15일). 개월 계산은 월말 오버플로를 말일로 클램프(1/31 + 1개월 → 2/28).
...
- `lib/prep-templates.ts` — 출장/여행/면접/병원/결혼식/시험 템플릿. 예: **출장 → 출장 신청서 작성(D-3)**·교통/숙소 예약(D-3)·짐 싸기(D-1)·당일 챙기기.
  - **여행**: 항공권 예약(D-60, 일찍 할수록 저렴) → 숙소(D-30) → 보험·환전(D-14) → 짐 리스트(D-3) → 짐 싸기(D-1) → 당일. 내년처럼 멀리 남은 여행이면 항공권 예약 마일스톤까지 포함되고, 임박한 일정이면 이미 지난 단계는 자동으로 제외된다(`filterByRoom` — 과거 날짜 할 일이 생기지 않도록 보장).
- `components/EventRow.tsx` — 제목 매칭 시 "📋 {유형} 준비 체크리스트" → 펼쳐보기 → 담기 → 각 항목이 `remindDate=일정일−daysBefore`, 09:00 알림. 유형 템플릿·일반 역산 계획 모두 `filterByRoom`으로 남은 기간에 맞는 항목만 담는다.
