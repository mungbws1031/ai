...
## 탭
- **🌱 쉽게 만들기(기본)**: 초보자용 3단계 위저드 — ① 느낌(무드)·지속 고르기 → ② 자동 생성된 향을 쉬운 말 설명과 함께 확인·다듬기(−/+) → ③ 실제 분량·준비물 안내. 용어 없이 따라만 하면 완성.
- **🧪 조합 스튜디오**: 직접 향료를 골라 만들고 분석·시뮬레이션·제작하는 본체(전문가용)
- **✨ 가상으로 만들기**: 상황(TPO)·계절·스타일/무드별 14가지 시나리오에서 "이럴 땐 이런 향" 추천(추천 무드·예상 지속·어울리는 향수) → 버튼 한 번으로 그 분위기 조합을 자동 생성해 스튜디오로 이동

## 기능
- **내 취향으로 추천**: 좋아하는 향수(프리셋 22종)를 고르면 그 향의 **계열 스타일·예상 지속**을 분석해, ① 원본 배합 불러오기 ② **같은 스타일의 비슷한 변형**(나만의 오리지널 레시피) 자동 생성 ③ **비슷한 향수 3종** 추천
...
- **재료 분량(정밀 제작)**: 용량(10~100ml)·농도(EDT/EDP/퍼퓸)·스포이트 보정(1ml당 방울 수)을 고르면 향료별 **무게(g)·ml·방울·비율(%)**, 향료/알코올/완성 총량을 g·ml로 계산하고, **저울 기준 정확한 제작 순서**(영점→베이스부터 계량→알코올로 총량 맞추기→숙성→여과)를 안내. 밀도(향료 0.90·에탄올 0.79 g/ml) 반영
